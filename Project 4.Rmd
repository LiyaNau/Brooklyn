...
```{r echo=FALSE,fig.height=8, warning=FALSE}
ggplot(filter(sales,gross.square.feet >0), aes(neighborhood, gross.square.feet)) + 
  geom_boxplot() + 
...
```{r echo=FALSE,warning=FALSE}
ggplot(filter(sales,sale.price > 0), aes(tax.class.at.present, sale.price/1000)) + 
  geom_boxplot() +
...
ggplot(filter(sales, sale.price >1000),
  ...
  geom_jitter(aes(color = tax.class.at.time.of.sale), width = 0.9, alpha = 0.2) +
  scale_y_log10()
```
...
ggplot(filter(sales, sale.price > 0),
  ...
  geom_boxplot() + 
  coord_flip(ylim = c(0,10000)) +
  scale_x_discrete(limits = rev(levels(sales$neighborhood))) +
  labs(x = "Neighborhoods", y = "Sale price for unite in thousands, $",
       title = "Distribution of sale price across neighborhoods")
```

### Description Two
Plot shows, as expected, that a lot of variance in sale prices depends on location

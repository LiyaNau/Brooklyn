...
sales <- read.xlsx("rollingsales_brooklyn.xlsx", sheet = 1, startRow = 5, detectDates = TRUE)
...
sales$building.class.at.time.of.sale <- as.factor(sales$building.class.at.time.of.sale)
...
one_dollar_sale <- sales %>%
  filter(sale.price == 1) %>%
  select(building.class.category,address, year.built, gross.square.feet, sale.price)
...
sales$price.square.foot <- 
   ifelse(sales$total.square.feet > 0, round(sales$sale.price /sales$total.square.feet, 2), 0)
```
...
```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(building.class.category)) + 
  geom_bar() + coord_flip()
```
...
```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(zip.code)) + geom_bar() + coord_flip()
```
...
### Sale date
I should have sales for whole 2015 year. 
...
Obviously most sales are made on week days with spike on June, 30 and decline to the end of year.
...
Here we can see some wertical bands for round numbers and a horizontal stripe around 1000000 and also a lot of variance in price for the same square feet.

Now I want to make the same for land :
...
Here also we can see strong vertical bands and a lot of differences in prices.
And now price vs. total square feet
...
Correlation is lower than 0.5  suggesting variables are correlated but not very strong.

Now I'm interested in comparing price per square foot vs. total sqare feet, may be there is a difference in price for small or lage properties.
...
Plot sows a lot of variance in price but no obvious increase or decrease.

let's look at price per foot vs  year.built
...
I can see more variance in prices for housed built in 1899-1931 and 2000-2015 years but I'm not sure of changes in mean prices so I want to cut years by decades and make a boxplot.

```{r echo=FALSE}
sales$year.built.bucket <- cut(sales$year.built, breaks = seq(1890,2020,10), dig.lab = 4)
```

```{r echo=FALSE}
ggplot(subset(sales,price.square.foot > 0), aes( year.built.bucket,price.square.foot)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(0,2000))
```

There are small differences in median price for different decades. On average properties that were built in the beginnig of this and last century cost more then those built in the middle of last century.

Obviously, a lot of difference in price per square foot is explaned by location:

```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(neighborhood, price.square.foot)) + 
  geom_boxplot() + 
...
```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(neighborhood, sale.price/(total.units * 1000))) + 
  geom_boxplot() + 
...
```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(neighborhood, gross.square.feet)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(1,20000)) + 
  coord_flip()
```

For example, properties that are sold  in Downtown - Fulton mall area are on average large than in Windsor Terrace. But if I look at area per unit  differences are not so large: 

```{r echo=FALSE,fig.height=8}
ggplot(sales, aes(neighborhood, gross.square.feet/total.units)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(1,20000)) + 
  coord_flip()
```
...
```{r echo=FALSE}
ggplot(sales, aes(tax.class.at.present, sale.price/1000)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(1,5000))
```
...
```{r echo=FALSE}
ggplot(sales, aes(price.square.foot)) + geom_density(aes(color = tax.class.at.present)) + 
  scale_x_continuous(limits = c(0.001,2000)) + scale_color_brewer(palette = "Dark2")
```

Distribution for class 2C looks rather different from others

```{r echo=FALSE}
ggplot(sales, aes(tax.class.at.present, price.square.foot)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(1,5000))
```
...
```{r echo=FALSE, fig.height=8}
ggplot(sales, aes(neighborhood) ) + geom_bar(aes(fill= tax.class.at.time.of.sale), position = "dodge") + 
...
There ise some difference in median prices for diffferent building classes. For example indoor public and cultural facilities cost per foot more then educational facilities

Sale price vs. sale date

```{r echo=FALSE}
ggplot(sales, aes(sale.date, sale.price/1000)) + geom_point(alpha = 1/5) +
  scale_x_date(date_breaks = '1 month') + scale_y_continuous(limits = c(0,50000))
```

Among big sales there are stripes aroun specific dates like 2015.03.01 or 2015.06.30.

```{r echo=FALSE}
ggplot(sales, aes(sale.date, sale.price/1000)) + 
  geom_point(alpha = 1/20) + 
  geom_line(color = 'red',stat = "summary", fun.y = mean) + 
  scale_x_date(date_breaks = '1 month') + scale_y_continuous(limits = c(1,10000))
```
...
```{r echo=FALSE}
ggplot(sales, aes(sale.date, price.square.foot)) + geom_point(alpha = 1/5) +

...
```{r echo=FALSE}
hist_base(sales$sale.price/1000, binwidth = 1000)
```
...
```{r echo=FALSE}
hist_base(sales$sale.price/1000, binwidth = 50) + xlim(0.001,5000)
```
...
```{r echo=FALSE}
hist_base(sales$gross.square.feet, bins = 500) +
...
plot + scale_x_continuous(limits = c(1, 10000), breaks = seq(0,10000,1000))
```
...
plot +  scale_x_continuous(limits = c(1, 25000))
```

And a closer look to values between 1 and 10000:

```{r echo=FALSE}
hist_base(sales$land.square.feet, bins = 200) +
...
```{r echo=FALSE}
plot <- hist_base(sales$total.square.feet, bins = 100)
```
 
Obviously I have here similar distribution with very long tale.

```{r  echo=FALSE}
plot + scale_x_continuous(limits = c(1,20000))
``` 
...
```{r echo=FALSE}
hist_base(sales$price.square.foot, binwidth = 50) +
...
```{r echo=FALSE}
hist_base(sales$price.square.foot, binwidth = 10) +
...
```{r echo=FALSE}
hist_base(sales$year.built, binwidth = 5) +
...
```{r echo=FALSE}
hist_base(sales$year.built, binwidth = 1) + 
  scale_x_continuous(limits = c(1800,2020), breaks = seq(1800,2020,20))
```
...
```{r echo=FALSE}
plot <- hist_base(sales$residential.units, binwidth = 1) 
plot
```

```{r echo=FALSE}
plot + scale_x_continuous(limits = c(0,50))
```
...
```{r echo=FALSE}
plot <- hist_base(sales$commercial.units, binwidth = 1)
plot
```

```{r echo=FALSE}
plot + scale_x_continuous(limits = c(0,20))
```
...
```{r echo=FALSE}
plot <- hist_base(sales$total.units, binwidth = 1)
plot
```
...
```{r echo=FALSE}
hist_base(sales$sale.date, binwidth = 1)
```

```{r echo=FALSE}
hist_base(sales$sale.date, binwidth = 7) +
